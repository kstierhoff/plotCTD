...
if (length(uctd.hdr) == 0) {
  ...
  uctd.hdr.missing <- all.uctd.hdr %>% 
    filter(!cast %in% uctd.summ$cast)
  
  # write table to CSV
  write.csv(uctd.hdr.missing, file = here("Output/unprocessed_uctd.csv"), quote = F, row.names = F)
  
  # summarize UCTD cast results
  uctd.summ <- uctd.summ %>%
    filter(!cast %in% uctd.hdr.missing$cast) %>%
