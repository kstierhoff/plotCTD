...
pacman::p_load(tidyverse,xlsx,cowplot,devtools,knitr,shadowtext,
          DBI,stringr,gdata,forecast,here,rnaturalearth,viridis)
...
if (get.nav) {
  ...
  nav <- filter(nav, is.nan(SOG) == F, SOG > 0, SOG < 12) %>%
...
map.bounds <- map_bounds(nav$lat, nav$long, 0.10)
...
base.map <- ggplot() +
  ...
  geom_shadowtext(data  = locations, aes(lon,lat, label = name), colour = 'gray20', size = 2,
                  fontface = 'bold', hjust = 0, nudge_x = 0.2, nudge_y = 0.05, angle = 25, bg.color = "white") +
...
# save plot
ggsave(base.map, filename = here("Figs/basemap.png"), width = map.width, height = map.height)
```
...
if (process.files) {
  ...
  uctd.files <- as.factor(gsub(".asc", "", list.files(here("Data/UCTD"),
                                                      paste(uctd.prefix, ".asc", sep = ""))))
  ...
  for (i in 1:length(uctd.proc)) {
    ...
    ucast <- read.table(uctd.proc[i],header = T)
  ...
  # save results 
  save(all.uctd, all.uctd.hdr, uctd.hdr, uctd.files, file = here("Data/uctd_data.Rdata"))
} else {
...
uctd.summ$cast.num <- as.numeric(substr(levels(uctd.summ$cast),
                                        nchar(levels(uctd.summ$cast)[1]) - 2,
                                        nchar(levels(uctd.summ$cast)[1])))
...
uctd.summ <- uctd.summ %>% 
  left_join(uctd.class[ ,c("cast","class")]) %>% 
  select(cast.num, cast, cast.date, lat, long, SOG, time, max.depth, class)
```
...
if (process.files) {
  ...
  ctd.hdr <- list.files(file.path(data.dir, "CTD"),
                        pattern = paste(ctd.prefix,".hdr", sep = ""),
                        full.names = T)
  ctd.files <- as.factor(gsub(".hdr", "", list.files(here("Data/CTD"), pattern = paste(ctd.prefix,".hdr", sep = ""))))
  ...
  for (i in 1:length(ctd.hdr)) {
    ...
    # extract cast date as POSIXct
    cast.date  <- as.POSIXct(str_extract(
      unlist(str_extract_all(txt, pattern = '\\* NMEA UTC \\(Time\\).*\\d{2}')),
  ...
  for (i in 1:length(ctd.proc)) {
    # read the ucast and rename columns
    cast <- read.table(ctd.proc[i], header = T)
    names(cast) <- c("P","T","C","Z","S","Sv","avgsVCM","Dens","Flag")
    # make depth negative
    cast$Z <- -cast$Z
    # add ucast name to df
    cast$cast <- ctd.files[i]
    # add to resultsConductivity Depth Plot
...
# extract cast number from filename
all.ctd.hdr$order.occ <- as.numeric(substr(levels(all.ctd.hdr$cast),nchar(levels(all.ctd.hdr$cast)[1]) - 2,
                                           nchar(levels(all.ctd.hdr$cast)[1])))
...
all.ctd.hdr <- select(all.ctd.hdr,order.occ,cast,cast.date,lat,long,max.depth,class)

# write table to CSV
write.csv(all.ctd.hdr, file = here("Output/cast_summary_ctd.csv"), quote = F, row.names = F)
```
...
if (draw.figures) {
  ...
  map.uctd <- base.map + 
    ...
    geom_text(data = uctd.table, aes(x = Lon, y = Lat, label = Cast), 
              size = 4, colour = "white", fontface = "bold") + 
    geom_text(data = uctd.table, aes(x = Lon, y = Lat, label = Cast),
              size = 3, colour = "red", fontface = "bold")  
  # save map
  ggsave(here("Figs/map_uctd.png"), map.uctd, width = map.width, height = map.height)  
}

include_graphics(here("Figs/map_uctd.png"))
```  
...
if (draw.figures) {
  ...
  uctd.dZt <- ggplot(data = all.uctd, aes(dZt, Z, group = cast, colour = SOG)) + 
    ...
    scale_x_continuous(limits = c(0,ceiling(max(all.uctd$dZt,na.rm = T))),
                       breaks = seq(0, ceiling(max(all.uctd$dZt,na.rm = T)), 0.5), expand = c(0,0)) + 
...
if (draw.figures) {
  ...
  uctd.Z <- ggplot(data = all.uctd, aes(s, Z, group = factor(cast), colour = SOG)) + 
    ...
    theme(panel.grid.major     = element_line(size = 0.75),
          ...
          legend.background    = element_blank()) +
    geom_smooth(data = all.uctd, se = F, aes(s, Z, group = survey), colour = 'black', linetype = 2)
  # save plot
...
if (draw.figures) {
  ...
  # save plot
  ggsave(here("Figs/map_T-S_uctd.png"), uctd.TS.map, width = map.width*2, height = map.height)
}
...
if (draw.figures) {
  ...
  map.ctd <- base.map + 
    geom_path(data = nav, aes(long, lat)) + 
    geom_text(data = all.ctd.hdr, aes(x = long, y = lat, label = order.occ),
              size = 4, colour = "white", fontface = "bold") + 
    geom_text(data = all.ctd.hdr, aes(x = long, y = lat, label = order.occ),
              size = 3, colour = "red", fontface = "bold") 
    
  # save map
  ggsave(here("Figs/map_ctd.png"), map.ctd, width = map.width, height = map.height)
}

include_graphics(here("Figs/map_ctd.png"))
```  
...
if (draw.figures) {
  ...
  # save plot
  ggsave(here("Figs/map_all_ctds.png"), both.maps, width = map.width*2, height = map.height)
}
```
...
if (draw.figures) {
  ...
  # save plot
  ggsave(ctd.TS.map,file = here("Figs/map_T-S_ctd.png"), width = map.width*2, height = map.height)
}
