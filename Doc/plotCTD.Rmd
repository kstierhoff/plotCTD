...
pacman::p_load(tidyverse,xlsx,cowplot,devtools,knitr,shadowtext,plotly,sf,
          DBI,stringr,gdata,forecast,lubridate,here,rnaturalearth,viridis,DT,mapview)
...
knitr::opts_chunk$set(
  echo = F, warning = F, message = F, out.width = "90%", out.height = "90%", 
...
if (length(uctd.hdr) > 0) {
  ...
  uctd.summ <- all.uctd %>% 
    ...
    mutate(cast.num = as.numeric(substr(levels(cast),
                                          nchar(levels(cast)[1]) - 2,
                                          nchar(levels(cast)[1]))),
  ...
  uctd.summ <- uctd.summ %>% 
    left_join(uctd.class[ ,c("cast","class")]) %>% 
    select(cast.num, cast, cast.date, lat, long, SOG, time, max.depth, class, leg)
}
```
...
if (length(ctd.hdr) > 0) {
  ...
  all.ctd.hdr <- all.ctd.hdr %>% 
    ...
    select(order.occ, cast, cast.date, lat, long, max.depth, class, leg)
