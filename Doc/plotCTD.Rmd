...
pacman::p_load(tidyverse,xlsx,cowplot,devtools,knitr,shadowtext,plotly,sf,
          DBI,stringr,gdata,forecast,here,rnaturalearth,viridis,DT,mapview)
...
knitr::opts_chunk$set(
  echo = F, warning = F, message = F, out.width = "100%",
...
uctd.files <- as.factor(gsub(".asc", "", 
                             list.files(here("Data/UCTD"),
...
ctd.hdr <- list.files(here("Data/CTD"),
                      pattern = paste(ctd.prefix,".hdr", sep = ""),
                      full.names = T)
ctd.files <- as.factor(gsub(".hdr", "", 
                            list.files(here("Data/CTD"),
                                       pattern = paste(ctd.prefix,".hdr", sep = ""))))
...
if (length(ctd.hdr) > 0) {
  ...
  # extract cast number from filename
  all.ctd.hdr$order.occ <- as.numeric(substr(levels(all.ctd.hdr$cast), 
                                             nchar(levels(all.ctd.hdr$cast)[1]) - 2,
  ...
  all.ctd.hdr <- select(all.ctd.hdr, order.occ, cast, cast.date, 
                        lat, long, max.depth, class)
...
uctd.sf <- st_as_sf(all.uctd.hdr, coords = c("long","lat"),crs = 4326) 
  ...
  if (draw.figures) {
    if (length(uctd.table) > 0) {
      ...
      map.uctd <- base.map + 
        ...
        geom_shadowtext(data = uctd.table, aes(x = Lon, y = Lat, label = Cast),
...
mapview(nav.sf, color = "black",  alpha = 0.5, layer.name = "Vessel track", col.regions = "black") +
  mapview(uctd.sf, color = "blue", cex = 4, layer.name = "UCTD Casts")
...
# Map casts
mapview(nav.sf, color = "black",  alpha = 0.5, layer.name = "Vessel track", col.regions = "black") +
  mapview(ctd.sf, color = "red", cex = 4, layer.name = "CTD Casts")
