...
reference_docx: _processCTD_template.docx
title: "1606RL UCTD Casts"
word_document: null
---

```{r global_options, include=FALSE}
knitr::opts_chunk$set(fig.width=5, fig.height=7, fig.align="center",
...
```{r DepthTimeRegression,eval=FALSE}
# # write.csv(all.uctd[ ,c("cast","s","Z","MX512.SOG")],file = "depth_time.csv",row.names = FALSE,quote = FALSE)
# x <- all.uctd$s[all.uctd$cast == "1606RL_UCTD_T001"]
# y <- all.uctd$Z[all.uctd$cast == "1606RL_UCTD_T001"]
# plot(x,y)
# a_start<- -1 #param a is the y value when x=0
...
# predicted.intervals <- as.data.frame(predicted.intervals)
# predicted.intervals <- predicted.intervals[order(-predicted.intervals$fit), ]
# plot(x,y,col='deepskyblue4',xlab='Time',main='Observed data')
# lines(x,predicted.intervals$fit,col='green',lwd=3)
# lines(x,predicted.intervals[,2],col='black',lwd=1)
# lines(x,predicted.intervals[,3],col='black',lwd=1)
```

**Table 1.** Summary of underway CTD (UCTD) casts conducted during the 2016 Summer CCE Survey aboard FSV _Reuben Lasker_. 
...
**Table 2.** Summary of traditional CTD casts conducted during the 2016 Summer CCE Survey aboard FSV _Reuben Lasker_. 
...
**Figure 1.** Location of underway UCTD (white numbers; left panel) and CTD (yellow numbers; right panel) casts. Gray numbers indicate acoustic transect numbers in the survey instructions. UCTD and CTD cast numbers correspond to those in **Table 1** and **Table 2**, respectively.  

```{r AllUctdMap,results='hide'}
# get map center
...
```{r AllCtdMap,results='hide'}
# create main map using ggmap
map.ctd <- uctd.map + 
  geom_path(data=wpt,aes(lon,lat,group=transect),colour = 'gray50') +
  geom_path(data=nav,aes(MX512.Lon,MX512.Lat)) + 
  geom_text(data=tx.labels,aes(x=lon,y=lat,label = transect),size=1.5,colour="gray50") + 
  geom_text(data=all.ctd.hdr,aes(x= MX512.Lon,y= MX512.Lat,label = order.occ),size=1.5,colour="yellow") + 
  xlab("\nLongitude (W)") + ylab("Latitude (N)\n") + 
  theme_bw() + theme(plot.background=element_blank(),axis.text.y = element_text(angle = 90, hjust=0.5)) + 
  coord_map(xlim=range.lon,ylim=range.lat)
# save map
...
ggsave("../Figs/map_all_ctds.png", both.maps,width = 8,height = 5,units = "in")
```

![](../Figs/map_all_ctds.png)

\newpage  

**Figure 2.** Descent rate of the UCTD by depth and vessel speed. The dashed line represents the Loess-smoothed descent rate.  
...
**Figure 3.** Maximum depth (m) of UCTD casts versus time. The dashed line represent the Loess-smoothed depth profiles.  
...
**Figure 4.** Temperature versus depth for each UCTD cast.  
...
**Figure 5.**  Salinity versus depth for each UCTD cast.
...
**Figure 6.**  Temperature-salinity plot for all UCTD casts (left) and the location of UCTD casts (right), symbolized by water mass.
...
**Figure 7.**  Sound speed versus depth for each UCTD cast.
...
**Figure 8.** Temperature versus depth for each CTD cast.  
...
**Figure 9.**  Salinity versus depth for each CTD cast.
...
**Figure 10.**  Temperature-salinity plot for all CTD casts (left) and the location of CTD casts (right), symbolized by water mass.
...
**Figure 11.**  Sound speed versus depth for each CTD cast.
