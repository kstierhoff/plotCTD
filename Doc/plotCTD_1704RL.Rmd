...
plotCTD.dir   <- "T:/SURVEYS/20170322_LASKER_SpringCPS/SOFTWARE/plotCTD"
survey.dir    <- "T:/SURVEYS/20170322_LASKER_SpringCPS"
data.dir      <- "T:/SURVEYS/20170322_LASKER_SpringCPS/DATA"

# Control script behavior
download.maps <- FALSE # download new maps, or use existing
copy.files    <- FALSE  # copy data files from data to plotCTD directory
process.files <- FALSE  # process CTD/UCTD files
draw.figures  <- FALSE # create new figures
...
wpt <- read.csv(file.path(survey.dir,"PLANNING/NAV/acoustic_waypoints.csv"))
# create labels for transects
tx.labels <- ddply(wpt,.(label),summarise,lon = min(lon) - 0.05,lat = min(lat) - 0.05)
```

```{r GetMapData}
# load or download boundaries
...
if(copy.files==TRUE){
  # Copy NAV data #####
  # file location on survey directory
  nav.files <- list.files(file.path(data.dir,"/SCS/CONTINUOUS"),pattern="*.elg",full.names = TRUE)
  # copy files to plotCTD directory
  file.copy(nav.files,file.path(plotCTD.dir,"Data/NAV"))
  
  # Copy UCTD data #####
  # location of header files on survey directory
  uctd.hdr <- list.files(file.path(data.dir,"UCTD"),pattern = "1704RL_UCTD_\\d{3}.asc",full.names = TRUE)
  # copy files to plotCTD directory
  file.copy(uctd.hdr,file.path(plotCTD.dir,"Data/UCTD"))
  # location of processed UCTD files on survey directory
  uctd.proc <- list.files(file.path(data.dir,"UCTD"),pattern = "1704RL_UCTD_\\d{3}_processed.asc",full.names=TRUE)
  # copy files to plotCTD directory
  file.copy(uctd.proc,file.path(plotCTD.dir,"Data/UCTD"))
  
  # Copy CTD data #####
  # list raw CTD ASCII files
  ctd.hdr <- list.files(file.path(data.dir,"CTD"),pattern = "^\\d{4}\\w{2}\\d{3}.hdr",full.names = TRUE)
  # copy files to plotCTD directory
  file.copy(ctd.hdr,file.path(plotCTD.dir,"Data/CTD"))
  ...
  # copy files to plotCTD directory
  file.copy(ctd.proc,file.path(plotCTD.dir,"Data/CTD"))
}
```
...
if(process.files==TRUE){
  # get nav file paths
  nav.files <- list.files(file.path(plotCTD.dir,"Data/NAV"),pattern="*.elg",full.names = TRUE)
  # process nav files
  nav <- data.frame()
  for(i in nav.files){
  ...
  # save results
  save(nav,file = file.path(plotCTD.dir,"Data/all_nav_data.Rdata"))
} else {
  load(file.path(plotCTD.dir,"Data/all_nav_data.Rdata"))
}
```
...
if(process.files==TRUE){
  ...
  uctd.hdr <- list.files(file.path(plotCTD.dir,"Data/UCTD"),pattern = "1704RL_UCTD_\\d{3}.asc",full.names = TRUE)
  uctd.files <- as.factor(gsub(".asc","",list.files(file.path(plotCTD.dir,"Data/UCTD"),pattern = "1704RL_UCTD_\\d{3}.asc")))
  ...
  uctd.proc <- list.files(file.path(plotCTD.dir,"Data/UCTD"),pattern = "1704RL_UCTD_\\d{3}_processed.asc",full.names=TRUE)
  ...
  # save results 
  save(all.uctd,all.uctd.hdr,uctd.hdr,uctd.files,file=file.path(plotCTD.dir,"Data/all_uctd_data.Rdata"))
} else {
  load(file.path(plotCTD.dir,"Data/all_uctd_data.Rdata"))
}
...
# write table to CSV
write.csv(uctd.summ,file=file.path(plotCTD.dir,"Data/cast_summary_uctd.csv"),quote = FALSE,row.names=FALSE)
# add SOG to all.uctd
...
```{r ProcessCTDCasts}
# list raw UCTD ASCII files
ctd.hdr <- list.files(file.path(plotCTD.dir,"Data/CTD"),pattern = "^\\d{4}\\w{2}\\d{3}.hdr",full.names = TRUE)
ctd.files <- as.factor(gsub(".hdr","",list.files(file.path(plotCTD.dir,"Data/CTD"),pattern = "^\\d{4}\\w{2}\\d{3}.hdr")))
...
if(process.files==TRUE){
  ...
  ctd.proc <- list.files(file.path(plotCTD.dir,"Data/CTD"),pattern = "^\\d{4}\\w{2}\\d{3}_processed.asc",full.names=TRUE)
  # create a data frame for storing results
  all.ctd <- data.frame()
  ...
  # save results
  save(all.ctd,all.ctd.hdr,file = file.path(plotCTD.dir,"Data/all_ctd_data.Rdata"))
} else {
  load(file.path(plotCTD.dir,"Data/all_ctd_data.Rdata"))
}
...
# write table to CSV
write.csv(all.ctd.hdr,file="../Data/cast_summary_ctd.csv",quote = FALSE,row.names=FALSE)
```
...
if(draw.figures==TRUE){
  ...
  # save map
  ggsave(file.path(plotCTD.dir,"Figs/map_uctd.png"), map.uctd,width = 5,height = 5)  
}
```  
...
if(draw.figures==TRUE){
  ...
  ggsave(uctd.dZt, file = file.path(plotCTD.dir,"Figs/uctd_dZt_v_Z.png"))
}
```
...
if(draw.figures==TRUE){
  ...
  ggsave(uctd.Z,file = file.path(plotCTD.dir,"Figs/uctd_Z_v_time.png"))
}
```  
...
if(draw.figures==TRUE){
  ...
  ggsave(uctd.T,file = file.path(plotCTD.dir,"Figs/uctd_T_v_Z.png"))
}
```
...
if(draw.figures==TRUE){
  ...
  uctd.S <- ggplot(data=filter(all.uctd,S>31),aes(S,Z,group=cast,colour=factor(cast))) + geom_path(alpha = 0.75) +
    ...
    theme_bw()+
    theme(panel.grid.major = element_line(size=0.75),legend.position = "none")
  ggsave(uctd.S,file = file.path(plotCTD.dir,"Figs/uctd_S_v_Z.png"))
}
```
...
if(draw.figures==TRUE){
  ...
  uctd.Sv <- ggplot(data=all.uctd,aes(Sv,Z,group=cast,colour=factor(cast))) + geom_path(alpha = 0.75) +
    ...
    theme_bw()+
    theme(panel.grid.major = element_line(size=0.75),legend.position = "none")
  ggsave(uctd.Sv,file = file.path(plotCTD.dir,"Figs/uctd_sV_v_T.png"))
}
```
...
if(draw.figures==TRUE){
  ...
  ggsave(file.path(plotCTD.dir,"Figs/map_T-S_uctd.png"), uctd.TS.map,width = 10,height = 5)
}
```
...
if(draw.figures==TRUE){
  ...
  map.ctd <- ggplot() + 
    ...
    coord_map(xlim=range.lon,ylim=range.lat)
  # save map
  ggsave(file.path(plotCTD.dir,"Figs/map_ctd.png"), map.ctd,width = 5,height = 5)
}
```  
...
if(draw.figures==TRUE){
  ...
  ggsave(file.path(plotCTD.dir,"Figs/map_all_ctds.png"),both.maps,width=10,height=5)
}
```
...
if(draw.figures==TRUE){
  ...
  ctd.T <- ggplot(data=all.ctd,aes(T,Z,group=cast,colour=factor(cast))) + geom_path(alpha = 0.75) +
    ...
    theme_bw() +
    theme(panel.grid.major = element_line(size=0.75),legend.position = "none")
  ggsave(ctd.T,file = file.path(plotCTD.dir,"Figs/ctd_T_v_Z.png"))
}
```
...
if(draw.figures==TRUE){
  ...
  ctd.S <- ggplot(data=all.ctd,aes(S,Z,group=cast,colour=factor(cast))) + geom_path(alpha = 0.75) +
    ...
    theme_bw()+
    theme(panel.grid.major = element_line(size=0.75),legend.position = "none")
  ggsave(ctd.S,file = file.path(plotCTD.dir,"Figs/ctd_S_v_Z.png"))
}
```
...
if(draw.figures==TRUE){
  ...
  ctd.Sv <- ggplot(data=all.ctd,aes(Sv,Z,group=cast,colour=factor(cast))) + geom_path(alpha = 0.75) +
    ...
    theme_bw()+
    theme(panel.grid.major = element_line(size=0.75),legend.position = "none")
  ggsave(ctd.Sv,file = file.path(plotCTD.dir,"Figs/ctd_sV_v_T.png"))
}
```
...
if(draw.figures==TRUE){
  ...
  ctd.TS <- ggplot(data=all.ctd,aes(S,T,group=cast,colour=factor(class))) + 
    ...
    theme(panel.grid.major = element_line(size=0.75),
          legend.position =  c(.2, .2),
          legend.background = element_rect())
  ggsave(ctd.TS,file = file.path(plotCTD.dir,"Figs/uctd_S_v_T.png"))
  ...
  ggsave(file.path(plotCTD.dir,"Figs/map_T-S_ctd.png"), ctd.TS.map,width = 10,height = 5)
}
```
